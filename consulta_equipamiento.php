...
$query = 'SELECT PRODUCTO.OID_PROV, PRODUCTO.NOMBRE, PRODUCTO.CODIGO, '
		...
		MOTOR.CILINDRADA, MOTOR.ESTADO, '
		. 'MOTOR.TIPOMOTOR, MOTOR.ANYOFABRICACION , MOTOR.GARANTIA '
		. 'FROM PRODUCTO, MOTOR '
		. 'WHERE '
		. 'PRODUCTO.OID_P = MOTOR.OID_P AND PRODUCTO.STOCK>PRODUCTO.STOCKMINIMO '
		. 'ORDER BY MARCA, NOMBRE';
}else{
	$query = 'SELECT PRODUCTO.OID_PROV, PRODUCTO.NOMBRE, PRODUCTO.CODIGO, '
			. 'PRODUCTO.DESCRIPCION, PRODUCTO.MARCA, PRODUCTO.TIPOPRODUCTO, PRODUCTO.OFERTA, '
			. 'PRODUCTO.IVA, PRODUCTO.STOCK, PRODUCTO.STOCKMINIMO, PRODUCTO.OID_P, '
			. 'PRODUCTO.PRECIO,EQUIPAMIENTO.TALLA, EQUIPAMIENTO.MATERIAL, EQUIPAMIENTO.COLOR '
			. 'FROM PRODUCTO, EQUIPAMIENTO '
			. 'WHERE '
			. 'PRODUCTO.OID_P = EQUIPAMIENTO.OID_P '
			. 'ORDER BY MARCA, NOMBRE';
}
...
<head>
  ...
  <title>Lista de productos de equipamiento</title>
</head>
...
<main>
	 ...
	 <nav id="navig">
		...
		<div id="enlaces">
			...
			<?php }	else { ?>
						...
						<a href="consulta_equipamiento.php?PAG_NUM=<?php echo $pagina; ?>&PAG_TAM=<?php echo $pag_tam; ?>"><?php echo $pagina; ?></a>
		...
		<form method="get" action="consulta_equipamiento.php">
	...
	<?php 	if(comprobar_tipo_usuario($conexion,$_SESSION['login'])!='Cliente'){?>
		<p id="registrar"><a href="form_alta_equipamiento.php">
	...
	<article class="productos">
		...
		<form method="post" action="controlador_equipamiento.php">
			...
			<div class="fila_productos">
				...
				<?php
					...
					== $fila["OID_P"])) { ?>
						...
						<!-- Editando título -->
<br>
						Precio:<input id="precio" name="precio" type="text" value="<?php echo $fila["PRECIO"]; ?>"/>€
<br><br>
...
<br><br>
						<h4><?php echo $fila["NOMBRE"] . " " . $fila["MARCA"] . " " . $fila["CODIGO"]; ?></h4>
	...
	<?php }?>
						...
						<?php }?>
						<div class="color">Color: <em><?php echo $fila["COLOR"]; ?></em></div>

						<div class="material">Material: <em><?php echo $fila["MATERIAL"]; ?></em></div>

						<div class="talla">Talla: <em><?php echo $fila["TALLA"]; ?></em></div>
